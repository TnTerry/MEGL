...
from transformers import LlavaProcessor, LlavaForConditionalGeneration, CLIPModel
...
def parse_args():
    ...
    parser.add_argument('--explanation_type', type=str, default="none", 
...
model = MEGL_ViT(args.explanation_type, clip_vit, num_classes)
...
for epoch in range(args.num_epochs):
    ...
    for i, data in enumerate(train_loader, 0):
        ...
        if args.explanation_type in ["multimodal", "visual"]:
            ...
            # Transform att loss
            visual_exp_resized_trans = visual_exp_transform(visual_exps_resized, args.transformation)
            att_loss = att_loss + cal_trans_att_loss(attn_weights, visual_exp_resized_trans, args.transformation)
        else:
